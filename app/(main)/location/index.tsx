...
export default function LocationScreen() {
  ...
  useRealtimeGuardian(guardianId, dashboardQuery.data?.driverIds ?? []);

  const liveLocationQuery = useGuardianLiveLocation(guardianId, dashboardQuery.data?.driverIds ?? []);
  ...
  const selectedLocation = useMemo(() => {
    ...
    return locations.find((location) => activeDriverIds.has(location.userId)) ?? locations[0] ?? null;
  ...
  const selectedDriver = useMemo(() => {
    ...
    return dashboardQuery.data?.drivers.find((driver) => driver.id === selectedLocation.userId) ?? null;
  ...
  if (profileQuery.isLoading || dashboardQuery.isLoading) {
    return (
      <SafeAreaView edges={['top', 'bottom']} style={{ flex: 1, backgroundColor: '#020617' }}>
        <View className="flex-1 items-center justify-center bg-slate-950 px-6">
          <ActivityIndicator size="large" color="#34d399" />
          <Text className="mt-4 text-base font-semibold text-white">Loading guardian live map...</Text>
        </View>
      </SafeAreaView>
    );
  }
        ...
        <View className="flex-1 justify-center bg-slate-950 px-5">
          <Card className="border border-amber-300 bg-amber-50">
            <CardContent className="p-4">
              <Text className="font-semibold text-amber-800">This screen is available to guardian accounts only.</Text>
              <Text className="mt-2 text-sm leading-6 text-amber-700">
                Sign in as a guardian to watch the live driver position during a HIGH_RISK event.
              </Text>
            </CardContent>
          </Card>
        </View>
      ...
      <View className="flex-1 bg-slate-950">
        {selectedLocation ? (
          ...
          <View className="absolute inset-0 items-center justify-center bg-slate-950 px-6">
            ...
            <Text className="mt-6 text-center text-2xl font-bold text-white">Waiting for live location</Text>
            <Text className="mt-3 max-w-sm text-center text-sm leading-6 text-slate-300">
              The map will activate when a linked driver enters HIGH_RISK and starts sending live location updates.
            </Text>
          </View>
        )}
          ...
          <View className="gap-3">
            {liveLocationQuery.error ? (
              <Card className="border border-red-300 bg-red-50">
                <CardContent className="p-4">
                  <Text className="font-semibold text-red-700">Unable to load live location.</Text>
